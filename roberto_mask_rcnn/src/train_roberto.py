...
# split the dataset continuously to thre parts
        
TRAIN_NUM = 5000
VAL_NUM = 500
TEST_NUM = DATA_NUM - VAL_NUM - TRAIN_NUM

# Training dataset
dataset_train = TrashDataset()
dataset_train.load_objects(0, TRAIN_NUM, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
dataset_train.prepare()

# Validation dataset
dataset_val = TrashDataset()
dataset_val.load_objects(TRAIN_NUM, TRAIN_NUM + VAL_NUM, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
dataset_val.prepare()

# Test dataset
dataset_test = TrashDataset()
dataset_test.load_objects(TRAIN_NUM + VAL_NUM, DATA_NUM, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
...
original_images = []
image_ids = np.random.choice(dataset_test.image_ids, model.config.BATCH_SIZE)
for image_id in image_ids:
...
for i in range(1):
    r = results[i]
    visualize.display_instances(original_images[i], r['rois'], r['masks'], r['class_ids'], 
...
# Running on a batch of images. Increase for better accuracy.
image_ids = np.random.choice(dataset_test.image_ids, 32)
images = []
APs = []
for image_id in image_ids:
